...
function ProposalCard({
  ...
  const supportPercent = totalVotes > 0
    ? Math.round(((proposal.supportVotes || 0) / totalVotes) * 100)
    : 50;
    ...
    >
      ...
      <View style={styles.cardHeader}>
        ...
        <View style={styles.headerRight}>
          {timeRemaining && (
            <View style={[styles.deadlineBadge, { backgroundColor: isEnded ? colors.errorLight : colors.goldLight }]}>
              <Ionicons
                name="time-outline"
                size={12}
  ...
  const filteredProposals = useMemo(() => {
    return proposals.filter((proposal) => {
      ...
      if (selectedGeoLevel !== 'All') {
        const geoTags = proposal.geoRestrictions || [];
        if (selectedGeoLevel === 'National')
          matchesGeo = geoTags.length === 0 || geoTags.some((t) => COUNTRIES.includes(t));
      ...
      if (selectedFilterGender !== 'All Genders')
        matchesGender =
          !demoRestrictions?.gender || demoRestrictions.gender === selectedFilterGender;
  ...
  const fetchData = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        if (claimedRes.data)
          setClaimedTokens(
            new Set(claimedRes.data.map((c: any) => (typeof c === 'object' ? c.proposalId : c)))
          );
        if (votedRes.data)
          setVotedProposals(
            new Set(votedRes.data.map((v: any) => (typeof v === 'object' ? v.proposalId : v)))
          );
        if (profileRes.data) {
    ...
    try {
      ...
      const result = await proposalsApi.create({
        ...
        demographicRestrictions:
          Object.keys(demographicRestrictions).length > 0 ? demographicRestrictions : undefined,
        imageUrl,
      });
      if (result.error) {
        Alert.alert('Error', result.error);
          ...
          <View style={styles.headerButtons}>
            ...
            >
              <Ionicons
                name="filter"
                size={18}
                color={hasActiveFilters ? '#000' : colors.text}
              />
            </TouchableOpacity>
            <TouchableOpacity
              ...
              <View style={styles.filterOptions}>
                {GEO_LEVELS.map((g) => (
                  <FilterChip
                    key={g}
                    label={g}
                    selected={selectedGeoLevel === g}
                    onPress={() => setSelectedGeoLevel(g)}
                  />
                ))}
              </View>
            </ScrollView>
      ...
      <Modal visible={showCreateModal} animationType="slide" presentationStyle="pageSheet">
        ...
        >
          ...
          <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
                  ...
                  <View style={styles.scopeOptions}>
                    <TouchableOpacity
                      style={[
                        ...
                        {
                          backgroundColor:
                            newProposal.geoScope === 'national' ? colors.goldLight : colors.cardBg,
                          borderColor:
                            newProposal.geoScope === 'national' ? colors.gold : colors.border,
                        },
                      ]}
                    ...
                    >
                      <Ionicons
                        ...
                        size={24}
                        color={
                          newProposal.geoScope === 'national' ? colors.gold : colors.textSecondary
                        }
                      />
                      <Text
                        style={[
                          ...
                          {
                            color:
                              newProposal.geoScope === 'national' ? colors.gold : colors.text,
                          },
                        ]}
                      >
                        National
                    ...
                    {userState && (
                      <TouchableOpacity
                        style={[
                          ...
                          {
                            backgroundColor:
                              newProposal.geoScope === 'state' ? colors.goldLight : colors.cardBg,
                            borderColor:
                              newProposal.geoScope === 'state' ? colors.gold : colors.border,
                          },
                        ]}
                      ...
                      >
                        <Ionicons
                          ...
                          size={24}
                          color={
                            newProposal.geoScope === 'state' ? colors.gold : colors.textSecondary
                          }
                        />
                        <Text
                          style={[
                    ...
                    {userCity && (
                      <TouchableOpacity
                        style={[
                          ...
                          {
                            backgroundColor:
                              newProposal.geoScope === 'city' ? colors.goldLight : colors.cardBg,
                            borderColor:
                              newProposal.geoScope === 'city' ? colors.gold : colors.border,
                          },
                        ]}
                      ...
                      >
                        <Ionicons
                          ...
                          size={24}
                          color={
                            newProposal.geoScope === 'city' ? colors.gold : colors.textSecondary
                          }
                        />
                        <Text
                          style={[
              ...
              >
                ...
                <View style={styles.warningContent}>
                  ...
                  <Text style={[styles.warningText, { color: colors.textSecondary }]}>
                    Complete identity verification to create geo-restricted proposals. Your proposal
            ...
            {/* Demographic Restrictions Section */}
            <View style={[styles.sectionDivider, { borderTopColor: colors.border }]}>
              <View style={[styles.sectionIcon, { backgroundColor: colors.goldLight }]}>
