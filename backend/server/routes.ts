...
// US licenses (and some Canadian documents) carry the region as a two-letter
// code ("NV", "AB"). Geo-gating string-matches the user's stored state against
// proposal geoRestrictions, which store full names ("Nevada") — an unmapped
// code makes every state-scoped proposal invisible to that user. Only exact
// two-letter inputs are mapped so real full names can never be mangled.
...
function normalizeStateName(s: any): string | undefined {
  if (!s || typeof s !== "string") return undefined;
  const v = s.trim();
  if (!v) return undefined;
  if (/^[A-Za-z]{2}$/.test(v)) return STATE_CODE_NAMES[v.toUpperCase()] ?? v;
  return v;
}
...
<body>
  ...
  // The Public Record — /record, /record/:region, and the redesigned /p/:id
